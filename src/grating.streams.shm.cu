...
#if STREAMS
cudaStream_t g_stStream[NUM_STREAMS] = {0};
#endif
...
int Init()
{
    ...
    {
        CUDASafeCallWithCleanUp(cudaStreamCreate(&g_stStream[i]));
    }
#endif
...
#if STREAMS
{
    struct rlimit stResLim = {0};
    /* increase the resource limit for locked memory - need to be root to do this */
    stResLim.rlim_cur = RLIM_INFINITY;
    stResLim.rlim_max = RLIM_INFINITY;
    iRet = setrlimit(RLIMIT_MEMLOCK, &stResLim);
    if (iRet != EXIT_SUCCESS)
    {
        (void) fprintf(stderr,
                       "ERROR: Setting page-lock limit failed: %s!\n",
                       strerror(errno));
        return EXIT_FAILURE;
    }
}
...
#if 0
    //CUDASafeCallWithCleanUp(cudaHostAlloc((void **) &g_pc4InBuf, g_iSizeFile, cudaHostAllocMapped));
    g_iShmID = shmget(IPC_PRIVATE, g_iSizeFile, 0444);
    if (EXIT_FAILURE == g_iShmID)
    {
        (void) fprintf(stderr,
    ...
    g_pc4InBuf = (char4 *) shmat(g_iShmID, NULL, 0);
    /*if ( EXIT_FAILURE = g_pc4InBuf)
    {
        (void) fprintf(stderr,
                       "ERROR: Failed to attach shared memory: %s!\n",
                       strerror(errno));
        return EXIT_FAILURE;
    }*/
    iRet = shmctl(g_iShmID, SHM_LOCK, NULL);
    if (iRet != EXIT_SUCCESS)
    {
        (void) fprintf(stderr,
                       "ERROR: Failed to lock shared memory: %s!\n",
                       strerror(errno));
        return EXIT_FAILURE;
    }
#endif
    iRet = mlock(g_pc4InBuf, g_iSizeFile);
    if (iRet != EXIT_SUCCESS)
    {
        (void) fprintf(stderr,
                       "ERROR: Failed to lock memory: %d: %s!\n",
...
#endif
#if STREAMS
    CUDASafeCallWithCleanUp(cudaMemcpyAsync(g_pc4Data_d,
                                            g_pc4InBufRead,
                                            g_iSizeRead,
                                            cudaMemcpyHostToDevice,
                                            0));
...
#ifdef STREAMS
//    (void) shmctl(g_iShmID, IPC_RMID, NULL);
{
    int iRet = EXIT_SUCCESS;
    iRet = munlock(g_pc4InBuf, g_iSizeFile);
    if (iRet != EXIT_SUCCESS)
    {
        (void) fprintf(stderr,
                       "ERROR: Failed to unlock memory: %d: %s!\n",
                       errno, strerror(errno));
    }
}
#endif
...
//#endif
    ...
    {
        /* check if it exists */
        cudaStreamDestroy(g_stStream[i]);
    }
#endif
