/** 
 * @file vegas_gpu_standalone_streams.cu
...
#include "vegas_gpu_standalone_streams.h"
...
char4* g_pc4InBufRead[NUM_STREAMS] = {NULL};
int g_iSizeFile = 0;
int g_iReadCount = 0;
char4* g_pc4Data_d[NUM_STREAMS] = {NULL};       /* raw data starting address */
char4* g_pc4DataRead_d[NUM_STREAMS] = {NULL};   /* raw data read pointer */
...
float4* g_pf4FFTIn_d[NUM_STREAMS] = {NULL};
float4* g_pf4FFTOut_d[NUM_STREAMS] = {NULL};
...
cudaStream_t g_stStreamCopy = 0;
cudaStream_t g_stStreamCompute = 0;
...
int main(int argc, char *argv[])
{
...
#endif
    int iBufNum = 0;
    cudaEvent_t stH2DDone;
    cudaEvent_t stProcDone;
#if OUTFILE
    int iFileSpec = 0;
#endif
    ...
    CUDASafeCallWithCleanUp(cudaEventCreate(&stH2DDone));
    CUDASafeCallWithCleanUp(cudaEventCreate(&stProcDone));
    ...
    /* create event in the copy stream so that the compute thread can check the
       event for synchronization */
    //CUDASafeCallWithCleanUp(cudaEventRecord(stH2DDone, g_stStreamCopy));
    ...
    {
        /* for stream synchronization */
        //CUDASafeCallWithCleanUp(cudaStreamWaitEvent(g_stStreamCompute,
        //                                            stH2DDone,
        //                                            0));
        if (g_iIsPFBOn)
        {
...
#endif
            DoPFB<<<g_dimGPFB, g_dimBPFB, 0, g_stStreamCompute>>>(g_pc4DataRead_d[iBufNum],
                                                                  g_pf4FFTIn_d[iBufNum],
                                                                  g_pfPFBCoeff_d);
            ...
            g_pc4DataRead_d[iBufNum] += (g_iNumSubBands * g_iNFFT);
        }
        else
        {
#if BENCHMARKING
            ...
            CUDASafeCallWithCleanUp(cudaEventSynchronize(g_cuStart));
#endif
            CopyDataForFFT<<<g_dimGCopy, g_dimBCopy, 0, g_stStreamCompute>>>(g_pc4DataRead_d[iBufNum],
                                                                             g_pf4FFTIn_d[iBufNum]);
            iCUDARet = cudaGetLastError();
            if (iCUDARet != cudaSuccess)
            {
        ...
        /* if time to read from the next input buffer */
            ...
            == iProcData)
        {
            ...
            {
                int iReadBufNum = iBufNum + 1;
                iReadBufNum %= NUM_STREAMS;
                /* read data from input buffer */
                iRet = ReadData(iReadBufNum);
                if (iRet != EXIT_SUCCESS)
                {
                    (void) fprintf(stderr, "ERROR: Data reading failed!\n");
                    break;
                }
                /* create event in the copy stream so that the compute thread
                   can check the event for synchronization */
                //CUDASafeCallWithCleanUp(cudaEventRecord(stH2DDone,
                //                                        g_stStreamCopy));
                iProcData = 0;
            }
        ...
        CUDASafeCallWithCleanUp(cudaEventSynchronize(g_cuStart));
#endif
        Accumulate<<<g_dimGAccum, g_dimBAccum, 0, g_stStreamCompute>>>(g_pf4FFTOut_d[iBufNum],
                                                                       g_pf4SumStokes_d);
        iCUDARet = cudaGetLastError();
        if (iCUDARet != cudaSuccess)
        {
        ...
        CUDASafeCallWithCleanUp(cudaDeviceSynchronize());
        /* create event in the copy stream so that the compute thread can check
           the event for synchronization */
        //CUDASafeCallWithCleanUp(cudaEventRecord(stProcDone,
        //                                        g_stStreamCompute));
        ...
        {
            /* for stream synchronization */
            //CUDASafeCallWithCleanUp(cudaStreamWaitEvent(g_stStreamCopy,
            //                                            stProcDone,
            //                                            0));
            /* dump to buffer */
#if BENCHMARKING
            ...
            CUDASafeCallWithCleanUp(cudaEventSynchronize(g_cuStart));
#endif
            CUDASafeCallWithCleanUp(cudaMemcpyAsync(g_pf4SumStokes,
                                                    g_pf4SumStokes_d,
                                                    (g_iNumSubBands
                                                     * g_iNFFT
                                                     * sizeof(float4)),
            ...
            iSpecCount = 0;
            /* zero accumulators */
            CUDASafeCallWithCleanUp(cudaMemsetAsync(g_pf4SumStokes_d,
                                                    '\0',
                                                    (g_iNumSubBands
                                                     * g_iNFFT
                                                     * sizeof(float4)),
    ...
    CUDASafeCallWithCleanUp(cudaEventDestroy(stH2DDone));
...
int Init()
{
    ...
    {
        CUDASafeCallWithCleanUp(cudaMalloc((void **) &g_pc4Data_d[i], g_iSizeRead));
        g_pc4DataRead_d[i] = g_pc4Data_d[i];
    }
    ...
    for (i = 0; i < NUM_STREAMS; ++i)
    {
        CUDASafeCallWithCleanUp(cudaMalloc((void **) &g_pf4FFTIn_d[i],
                                           g_iNumSubBands
                                           * g_iNFFT
                                           * sizeof(float4)));
        CUDASafeCallWithCleanUp(cudaMalloc((void **) &g_pf4FFTOut_d[i],
                                           g_iNumSubBands
                                           * g_iNFFT
                                           * sizeof(float4)));
    }
    ...
    CUDASafeCallWithCleanUp(cudaStreamCreate(&g_stStreamCopy));
    CUDASafeCallWithCleanUp(cudaStreamCreate(&g_stStreamCompute));

    /* associate the CUFFT plan with the compute stream */
    iCUFFTRet = cufftSetStream(g_stPlan, g_stStreamCompute);
    if (iCUFFTRet != CUFFT_SUCCESS)
    {
        (void) fprintf(stderr, "ERROR: Plan association with stream failed!\n");
        return EXIT_FAILURE;
    }
...
int LoadDataToMem()
{
    ...
    for (i = 0; i < NUM_STREAMS; ++i)
    {
        g_pc4InBufRead[i] = g_pc4InBuf + ((i * g_iSizeRead) / sizeof(char4));
    }
...
int ReadData(int iBufNum)
{
    ...
    CUDASafeCallWithCleanUp(cudaEventSynchronize(g_cuStart));
#endif
    CUDASafeCallWithCleanUp(cudaMemcpyAsync(g_pc4Data_d[iBufNum],
                                            g_pc4InBufRead[iBufNum],
                                            g_iSizeRead,
                                            cudaMemcpyHostToDevice,
                                            g_stStreamCopy));
#if BENCHMARKING
    ...
    /* update the read pointer to where data needs to be read in from, in the
       next read */
    g_pc4InBufRead[iBufNum] += ((g_iSizeRead
                                 - ((g_iNTaps - 1)
                                    * g_iNumSubBands
                                    * g_iNFFT
                                    * sizeof(char4)))
    ...
    g_pc4DataRead_d[iBufNum] = g_pc4Data_d[iBufNum];
    ++g_iReadCount;
    /* BUG: won't read last block */
    if ((((char *) g_pc4InBuf) + g_iSizeFile) - ((char *) g_pc4InBufRead[iBufNum])
        <= g_iSizeRead)
    {
...
int DoFFT(int iBufNum)
{
    cufftResult iCUFFTRet = CUFFT_SUCCESS;

    /* execute plan */
    iCUFFTRet = cufftExecC2C(g_stPlan,
                             (cufftComplex*) g_pf4FFTIn_d[iBufNum],
                             (cufftComplex*) g_pf4FFTOut_d[iBufNum],
                             CUFFT_FORWARD);
    if (iCUFFTRet != CUFFT_SUCCESS)
    {
        (void) fprintf(stderr, "ERROR! FFT failed!\n");
...
void CleanUp()
{
    ...
    for (i = 0; i < NUM_STREAMS; ++i)
    {
        if (g_pc4Data_d[i] != NULL)
        {
            (void) cudaFree(g_pc4Data_d[i]);
            g_pc4Data_d[i] = NULL;
        }
        if (g_pf4FFTIn_d[i] != NULL)
        {
            (void) cudaFree(g_pf4FFTIn_d[i]);
            g_pf4FFTIn_d[i] = NULL;
        }
        if (g_pf4FFTOut_d[i] != NULL)
        {
            (void) cudaFree(g_pf4FFTOut_d[i]);
            g_pf4FFTOut_d[i] = NULL;
        }
    }
    ...
    cudaStreamDestroy(g_stStreamCopy);
    cudaStreamDestroy(g_stStreamCompute);
